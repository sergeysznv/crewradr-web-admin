    ...
    <div className="fixed bottom-20 md:bottom-6 left-1/2 -translate-x-1/2 z-30 bg-surface border border-outline rounded-xl shadow-sm px-sz-lg py-3 flex items-center gap-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <select
          ...
          aria-label={t('webMembersBulkRole')}
        >
      ...
      >
        ...
        {t('webMembersBulkRemove')}
      </button>
      <button onClick={onClear} disabled={working} className="text-xs text-on-surface-variant underline disabled:opacity-50">{t('webMembersBulkClear')}</button>
    </div>
  );
}
