      ...
      <div className="fixed inset-0 flex items-center justify-center z-50 pointer-events-none">
        <div className="bg-surface border border-outline rounded-xxl p-xl max-w-[520px] w-full mx-lg pointer-events-auto shadow-xl">
          <div className="flex items-center justify-between mb-lg">
            <h2 className="font-heading font-extrabold text-lg text-on-surface">Import Members</h2>
          ...
          <div className="border-2 border-dashed border-outline rounded-lg p-xl text-center mb-lg">
            ...
            <p className="text-sm text-on-surface-variant">Paste CSV data below</p>
            <p className="text-2xs text-on-surface-variant mt-1">Format: email, role (member or co-captain)</p>
          </div>

          <textarea value={text} onChange={e => setText(e.target.value)}
            placeholder="email,role&#10;sarah@example.com,member&#10;mike@example.com,co-captain"
          ...
          <div className="flex gap-3 mt-lg justify-end">
            <button onClick={onClose}
              className="px-4 py-2 rounded-xl border border-outline text-sm font-semibold text-on-surface-variant hover:bg-surface-container">Cancel</button>
          ...
          {importMutation.data && (
            <div className={`mt-lg p-3 rounded-lg text-sm ${importMutation.data.errors.length ? 'bg-warning-container text-warning' : 'bg-primary-container text-primary'}`}>
              Added {importMutation.data.added} members
